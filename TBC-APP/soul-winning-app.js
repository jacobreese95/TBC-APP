...
function statusBadge(status) {
  ...
  if (s === 'accepted' || s === 'available') label = 'Accepted';
  if (s === 'declined' || s === 'unavailable') label = 'Declined';
...
function renderTerritoriesList() {
  ...
  el.innerHTML = list.map(function (t) {
    var peopleLine = (t.assignedPeople || []).map(function (p) {
      var st = p.responseStatus || p.status || 'pending';
      return escapeHtml(p.name) + ' ' + statusBadge(st);
    }).join('<br>');
    ...
    return '<div class="territory-item">' +
      ...
      '<div style="font-size:0.85rem;color:#7a8fac;margin-bottom:6px">' + statusBadge(t.status || 'open') + '</div>' +
      '<div style="font-size:0.9rem;margin-bottom:6px"><strong>Assigned:</strong><br>' + (peopleLine || '—') + '</div>' +
...
async function saveTerritory() {
  ...
  try {
    var ref = await db.collection('soulWinningTerritories').add({
      ...
      notes: notes,
      points: pendingShape,
      assignedPeople: terrSelected.map(function (p) { return { uid: p.uid, name: p.name, responseStatus: 'pending' }; }),
...
async function deleteTerritory(id) {
  ...
  try {
    ...
    try {
      var rs = await db.collection('soulWinningAssignments').where('territoryId', '==', id).get();
      var batch = db.batch();
...
function loadMyAssignments() {
  ...
  var uid = auth.currentUser.uid;
  db.collection('soulWinningAssignments')
    .where('assignedToUid', '==', uid)
    .where('status', '==', 'pending')
    .onSnapshot(function (snap) {
      myAssignments = [];
      snap.forEach(function (doc) {
        myAssignments.push(Object.assign({ id: doc.id }, doc.data()));
      });
      renderMyAssignments();
    }, function (err) {
      console.warn('my assignments', err);
      db.collection('soulWinningAssignments')
        .where('assignedToUid', '==', uid)
        .onSnapshot(function (snap2) {
          ...
          snap2.forEach(function (doc) {
            ...
            myAssignments.push(Object.assign({ id: doc.id }, d));
          });
          renderMyAssignments();
        }, function (e2) { console.warn(e2); });
    });
}

function renderMyAssignments() {
  var section = document.getElementById('my-assignments-section');
  var list = document.getElementById('my-assignments-list');
  ...
  list.innerHTML = myAssignments.map(function (a) {
    return '<div class="req-card">' +
      '<h4 style="margin:0 0 6px">' + escapeHtml(a.territoryName || 'Soul winning area') + '</h4>' +
      '<div style="font-size:0.85rem;color:#7a8fac;margin-bottom:6px">Assigned by ' + escapeHtml(a.createdByName || 'Leader') + '</div>' +
      (a.notes ? '<p style="margin:0 0 8px;font-size:0.9rem">' + escapeHtml(a.notes) + '</p>' : '') +
      '<p style="margin:0 0 8px">Can you cover this area?</p>' +
      '<div class="req-actions">' +
      '<button type="button" class="btn-available" onclick="respondTerritoryAssignment(\'' + a.id + '\',\'accepted\',\'' + (a.territoryId || '') + '\')">Accept</button>' +
      '<button type="button" class="btn-unavailable" onclick="respondTerritoryAssignment(\'' + a.id + '\',\'declined\',\'' + (a.territoryId || '') + '\')">Decline</button>' +
      '</div></div>';
  }).join('');
}

async function respondTerritoryAssignment(assignmentId, status, territoryId) {
  if (!auth.currentUser) return;
  try {
    await db.collection('soulWinningAssignments').doc(assignmentId).update({
      status: status,
      respondedAt: firebase.firestore.FieldValue.serverTimestamp()
    });
    if (territoryId) {
      try {
        var ref = db.collection('soulWinningTerritories').doc(territoryId);
        var snap = await ref.get();
        if (snap.exists) {
          var data = snap.data() || {};
          var people = (data.assignedPeople || []).map(function (p) {
            if (p.uid === auth.currentUser.uid) {
              return Object.assign({}, p, { responseStatus: status });
            }
            return p;
          });
          await ref.update({ assignedPeople: people });
        }
      } catch (e) {
        console.warn('territory person status', e);
      }
    }
    showToast(status === 'accepted' ? 'Area accepted — thank you!' : 'Area declined.');
    switchTab('map');
    setTerritoryFilter('mine');
  } catch (err) {
    showToast('Error: ' + err.message);
  }
}
