...
(function () {
  ...
  async function collectNotifications(user, profile) {
    var fdb = (typeof db !== 'undefined') ? db : window.db;
    if (!fdb || !user) return [];
    var items = [];
    var uid = user.uid;
    ...
    try {
      var pend = await fdb.collection('scheduleRequests')
      ...
      console.warn('notif pending composite', e);
      try {
        var pend2 = await fdb.collection('scheduleRequests')
          .where('assignedToUid', '==', uid)
          .get();
        pend2.forEach(function (doc) {
          var d = doc.data() || {};
        ...
        console.warn('notif pending fallback', e2);
      }
    }

    if (isLeader) {
      try {
        var respSnap = await fdb.collection('scheduleRequests')
    ...
    for (var i = 0; i < rooms.length; i++) {
      var room = rooms[i];
      try {
        var msgSnap = await fdb.collection('chats').doc(room).collection('messages')
  ...
  async function refreshNotifications(renderPanel) {
    ...
    if (typeof db === 'undefined' && !window.db) {
      setTimeout(function () { refreshNotifications(renderPanel); }, 400);
      return;
    }
    try {
      var profile = null;
